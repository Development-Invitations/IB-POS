...
// Настоящих API/SDK этих касс (Epos, SmartPos, ArcaGroup, RahmatPos) у нас нет — у каждого
// клиента будет своя касса и свои учётные данные, протокол выбирается на внедрении (см. ТЗ
// Этап 5). До тех пор — честная симуляция по общему контракту IntegrationAdapter: проверяем
// наличие логина/токена в конфиге, а "отправку чека" эмулируем с задержкой, чтобы можно было
// разрабатывать и тестировать очередь синка уже сейчас. Regos с реальным протоколом ушёл в
// свой адаптер — см. regos-adapter.ts и registry.ts.
