...
// Настоящих API/SDK этих касс (ArcaGroup, RahmatPos) у нас нет — у каждого клиента будет своя
// касса и свои учётные данные, протокол выбирается на внедрении (см. ТЗ Этап 5). До тех пор —
// честная симуляция по общему контракту IntegrationAdapter: проверяем наличие логина/токена
// в конфиге, а "отправку чека" эмулируем с задержкой, чтобы можно было разрабатывать и
// тестировать очередь синка уже сейчас. Regos и Epos с реальными протоколами ушли в свои
// адаптеры — см. regos-adapter.ts, epos-adapter.ts и registry.ts.
